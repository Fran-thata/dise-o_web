          ...
          {sectors.map((sector, index) => {
            ...
            return (
              <Reveal 
                key={index} 
                delay={index * 50}
                width={isLast ? "100%" : "fit-content"}
                className={isLast ? "md:col-span-2 lg:col-span-1 lg:col-start-2 flex justify-center lg:block" : ""}
              >
                <Link 
                  to={sector.link}
                  className={`p-6 rounded-2xl bg-white/5 border border-white/5 hover:border-white/10 transition-all group block ${isLast ? 'w-full max-w-md lg:max-w-none' : ''}`}
                >
